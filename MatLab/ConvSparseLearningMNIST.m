...
for epoch = 1:maxepoch
    ...
    for loop = 1:nloop %1:100
        
        count = count + 1;
        
        if batch_model == 1show
...
%             img = squeeze(xtrain(:,:,:,index(loop)));
        ...
        if count > 10
         ...
         if ~mod(count, 100)
             ...
             if strcmpi(datasetName,'CIFAR10')
                 ...
                 display_network(kernels, true, true, 3);  title 'Słownik D'
             end
            % figure(3); subplot(121), imshow(img,[]);  title 'Oryginał'%imagesc(img); title 'Original image'
             ...
             %figure(4); montage(enc); colormap bone; colorbar; title 'Mapa współczynników'
         end
    ...
    %saveas(figure(3), "D:\Studia\PracaMagisterska\MatLab\CSAEs-master\MNIST\images\rekonstrukcja" + epoch + ".png"); 
    %saveas(figure(4), "D:\Studia\PracaMagisterska\MatLab\CSAEs-master\MNIST\images\mapa" + epoch + ".png"); 
end
saveas(figure(2), "D:\Studia\PracaMagisterska\MatLab\CSAEs-master\MNIST\images\" + kernelsize1 + "x" + kernelsize2 + "x" + numfeatures1 + "\lmbda_" + tiedparams.lambda + "\slownik.png"); 
